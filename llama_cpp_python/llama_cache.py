...
class BaseLlamaCache(ABC):
    ...
    @abstractmethod
    def __getitem__(self, key: Sequence[int]) -> "llama_cpp_python.llama.LlamaState":
        raise NotImplementedError
    ...
    @abstractmethod
    def __setitem__(
        self, key: Sequence[int], value: "llama_cpp_python.llama.LlamaState"
    ) -> None:
        raise NotImplementedError
...
class LlamaRAMCache(BaseLlamaCache):
    ...
    def __init__(self, capacity_bytes: int = (2 << 30)):
        ...
        self.capacity_bytes = capacity_bytes
        self.cache_state: OrderedDict[Tuple[int, ...], "llama_cpp_python.llama.LlamaState"] = (
            OrderedDict()
        )
    ...
    def _find_longest_prefix_key(
        ...
        min_key = None
        keys = (
            (k, llama_cpp_python.llama.Llama.longest_token_prefix(k, key))
    ...
    def __setitem__(self, key: Sequence[int], value: "llama_cpp_python.llama.LlamaState"):
        key = tuple(key)
...
class LlamaDiskCache(BaseLlamaCache):
    ...
    def _find_longest_prefix_key(
        ...
        for k in self.cache.iterkeys():  # type: ignore
            prefix_len = llama_cpp_python.llama.Llama.longest_token_prefix(k, key)
            if prefix_len > min_len:
                min_len = prefix_len
